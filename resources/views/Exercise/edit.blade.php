...
@section('main')
    <form class="grid-x grid-padding-x grid-padding-y bg-white px-2" method="POST" action="{{route('exercise.update', $exercise)}}">
        @csrf
        @method('PUT')
        @if (count($errors) > 0)
        ...
        <div class="cell small-12 medium-10">
            <input id="question" name="question" type="text" class="my-0" required value="{{$exercise->question}}">
        </div>
    ...
    <script>
        document.querySelector('select[name="questionType"] option[value="{{$exercise->questionType->id ?? ''}}"]').setAttribute('selected', 'selected');
    </script>
@endsection
